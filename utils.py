...
def generate_script(subject,video_length,creativity,api_key):
    ...
    script_template = ChatPromptTemplate.from_messages(
        [
            (
                "human",
                """你是一位优秀的短视频频道博主。根据以下标题和相关信息，为短视频频道写一个视频脚本。
                视频标题：{title},视频时长{duration}分钟，生成的脚本的长度尽量遵循视频时长的要求。
                要求开头抓住眼球，中间提供干货内容，结尾有惊喜，脚本格式也请按照【开头,中间,结尾】分割。
                整体内容的表达要尽量轻松有趣,幽默诙谐,吸引年轻人的注意。
                脚本内容可以结合以下维基百科搜索出的信息，但仅供参考，只结合相关的内容即可，对不相关的内容进行忽略:
                ```{wikipedia_search}```
                """
            )
        ]
...
# print(generate_script("sora模型", 1 ,0.7, os.getenv("OPEN_AI_KEY")))